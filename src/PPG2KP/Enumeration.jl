...
> L÷2 ∧ li < L | L - li]`; assumes `l` is in increase-or-stay order.

The new value of `l` has an important property that: any pair of plates
generated by a cut using the original `l` will also be generated by some cut in
the modified set, even with the modified set only having cuts in the first half
of the plate. Note any values equal to or above L are discarded.
"""
function reflect!(l, L)
	mid_L = L ÷ 2
	qt_elems = 0
	for (i, v) in pairs(l)
		v >= L && break
...
# !!! **Internal use.**
#
#     _pop_smaller_than!(l, min_l)
#
# Assumes a increase-or-stay ordered `Vector` `l` and delete values smaller
# than `min_l`. Returns `l`.
function _pop_smaller_than!(l, min_l)
	# This may seem terrible performance-wise, but the way Julia stores
	# `Vectors` allows `popfirst!` to be on par with `pop!`.
	while !isempty(l) && first(l) < min_l
		popfirst!(l)
	end
	return l
end
...
function gen_cuts(
	...
	while next_idx <= length(next)
		...
		if !no_cut_position
			# In the code below, we had a long-standing bug in the interaction
			# between the Cut-Position dominance and the enhanced model. The bug only
			# occured in very specific circunstances, i.e., ALL optimal solutions
			# must depend on (i) the existence of an intermediary plate that does not
			# fit 6 pieces and must also depend on (ii) the existence of a cut over
			# such plate that is not the size of any piece smaller than half the
			# plate.  For example, in the specific case we found this bug we had a
			# 2x12 plate that fitted at most 5 pieces (and, in fact, in the studied
			# optimal packing it had 5 pieces inside), and the first cut to be done
			# had to divide it from 2x12 into 2x4 and 2x8. The Cut-Position changed
			# dw1 to [1, 2, 3, 8, 12], the original discretization had a cut at 4
			# because 1 + 3, but as Cut-Position only allows restricted cuts and
			# there is no piece of width 4. In the faithful/original model this is
			# not a problem, because every position in the array is considered, but
			# the same is not true for enhanced: only positions before the middle of
			# the plate are cut. Disabling Cut-Position for the enhanced formulation
			# is an obvious fix, but there are many cases in which the Cut-Position
			# and the enhanced formulation do not conflict, and it is in our interest
			# to reduce every cut possible while guaraneeing optimality. The solution
			# found was to "reflect" the restricted cut set to the first half of the
			# plate; so, if some piece larger-than-half-plate needs to be cut from a
			# do-not-fit-6-piece plate, then there will be a cut in the first half
			# that leaves a second child of the exact length (or width) of the piece
			# we need to obtain. It is also possible to compare the enhanced cut set
			# without Cut-Position with this restricted-"reflected" one, and chose
			# the best (i.e., the smallest). The advantage of the
			# restricted-"reflected" approach is to keep the assumption the cuts will
			# only be done at the first half of the plate (from the enhanced
			# formulation), while univocally having the same benefit than the one
			# given by the original restricted approach (in fact, given the removal
			# of perfect symmetries used by the faithful formulation, things like
			# the vertical cuts 4 and 8 in a plate of width 12 were already being
			# conflated into a single cut).
			if no_chance_to_fit_6_piece(
				d, l, w, a, pll, plw, pll * plw; ignore_2th_dim = ignore_2th_dim
			)
				if ignore_2th_dim
					# This `deepcopy` is a ridiculous solution, but `ignore_2th_dim' is
					# never used seriously and I refuse to create yet another variable
					# just for it.
					dl1 = _pop_smaller_than!(reflect!(deepcopy(usl), pll), min_pil)
					dw1 = _pop_smaller_than!(reflect!(deepcopy(usw), plw), min_piw)
				else
					dl1 = reduce2fit_usl(sllw.sl, sllw.sli2pii, w, pll, plw)
					@assert dl1 ⊆ dl2
					dw1 = reduce2fit_usl(sllw.sw, sllw.swi2pii, l, plw, pll)
					@assert dw1 ⊆ dw2
					# Here is the workaround we talk about above.
